...
def ftp_walk(path):
    ...
    for program in ftp.nlst():
        if 'svg' in program:
            continue
        if '.' not in program:
            name, program_type, version = program, 'Folder', ''
        else:
            name, program_type, version = program.rsplit('.', 1)[
                0].rsplit(' ', 2)
        items[name] = {
            'type': program_type,
            'version': version,
...
full_list = dumps(
    ftp_walk('3c3f5e3a-30e2-d801-3036-5e3a30e2d801/debian/root/collection'))
local_file = '/'.join(abspath(__file__).split('/')
                      [:-1]) + '/../pages/collection.json'
