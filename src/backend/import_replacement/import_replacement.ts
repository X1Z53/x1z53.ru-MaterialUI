...
const database: DatabaseRow[] = parse("import_replacement.xlsx")[0]
    .data
    .slice(1)
    .map((row: string[]) => ({
...
const toDelete = (file: string): void => readline.question(
    `Delete ${file}? `,
    (toDelete: string): void => (toDelete == 'y') ? fs.unlinkSync(file) : console.log('File not deleted')
)
